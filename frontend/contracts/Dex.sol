...
contract Orderbook {
    ...
    function placeBuy(uint256 _price, uint256 _quantity, address _buyer) external {
        ...
        uint256 x;
        sortedByDate.push(_buyer);

        // block.timestamp
        // put in sortedByDate 
        //  checkExpire(sortedByDate) reorder LL
        //         loop if(date == x + block.timestamp){
        //             cancelBuy(Order.addres) ..
        //         }
        //         reorder link
    ...
    function checkExpire(address[] sortedByDate, uint256 x) internal view returns (address[]) {
        uint256 index;
        index = sortedByDate.length;
        
        // for(int i = sortedByDate.length; i>0; i--){
            // if(sortedByDate[i-1] == block.timestamp){
        // console.log("haha", block.timestamp);
            // }
        // }
    }
